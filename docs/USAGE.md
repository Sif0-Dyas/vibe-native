...
- **☼ solar** — one saved **playlist** as a solar system. The dropdown next to the mode buttons picks which playlist is the sun, and it keeps up with the playlist panel: **save a playlist while the Map is open and it appears in the list right away.** The playlist's name is the sun, and every track orbits it. The **rings** dropdown decides what distance means: *fit* (how close a track is to the playlist's overall sound — the core hugs the sun, the odd ones out orbit far away), *tempo*, or *genre*. **Use this to** sanity-check a set: anything way out on the last ring probably doesn't belong.
